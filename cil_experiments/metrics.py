...
def validate_summary(
    ...
    if set(summary) != SUMMARY_KEYS:
        raise ValueError(f"summary top-level keys differ: {set(summary) ^ SUMMARY_KEYS}")
    if set(summary["cil_performance"]) != CIL_KEYS:
        raise ValueError("cil_performance keys differ from metrics1.docx")
    if set(summary["network"]) != {"final_hidden_neurons", "total_new_neurons", "total_reused_neurons"}:
        raise ValueError("network keys differ from metrics1.docx")
    if set(summary["training_runtime"]) != {
        ...
        raise ValueError("training_runtime keys differ from metrics1.docx")
    ...
    if set(summary["training_operations"]) != training_operation_keys:
        raise ValueError("training_operations keys differ from metrics1.docx")
    ...
    for key in FLOP_KEYS:
        if isinstance(flops[key], bool) or not isinstance(flops[key], int) or flops[key] < 0:
            raise ValueError(f"flops.{key} must be a non-negative integer")
    ...
    if flops["overall_learning_flops"] != formal_training_flops:
        raise ValueError("overall learning FLOPs identity failed")
    ...
    if set(nonflop) != {
        ...
        raise ValueError("auxiliary_nonflop_ops keys differ")
    ...
    if not isinstance(calls, dict) or any(
        ...
        raise ValueError("auxiliary_nonflop_ops call counts are invalid")
    ...
    if not isinstance(manual_operations, list) or any(
        ...
        raise ValueError("manual auxiliary non-FLOP records are invalid")
    ...
    if nonflop["total_recorded_events"] != expected_nonflop_events:
        raise ValueError("auxiliary_nonflop_ops total_recorded_events mismatch")
    if not isinstance(nonflop["policy"], str) or not nonflop["policy"]:
        raise ValueError("auxiliary_nonflop_ops policy must be a non-empty string")
    ...
    if not math.isclose(runtime["total_s"], runtime["initial_task_s"] + runtime["incremental_tasks_total_s"], rel_tol=1e-8, abs_tol=1e-8):
        raise ValueError("runtime decomposition failed")
