...
def loss_flop(input_shape, target_shape=None, *args, out_shape=None, **kwargs) -> int:
    return max(_numel(input_shape), _numel(target_shape))
...
def build_custom_mapping() -> dict[Any, Callable[..., int]]:
    ...
    add("_adaptive_avg_pool2d_backward", adaptive_pool_flop)
    ...
    for name in ("nll_loss_forward", "nll_loss_backward", "binary_cross_entropy", "binary_cross_entropy_backward", "mse_loss", "mse_loss_backward"):
        add(name, loss_flop)
